"""
Leslie Smith's LR Range Test for Darcy_FNO.
...
from Darcy_FNO import LpLoss, MatRead, UnitGaussianNormalizer, FNO

# ─── Hyperparameters ───────────────────────────────────────────────────────────
MODES      = 4      # must match the model you intend to train
WIDTH      = 7      # must match the model you intend to train
LR_START   = 1e-7
LR_END     = 1.0
...
def plot_lr_finder(lrs, losses, save_path='fno_lr_finder.png'):
    fig, ax = plt.subplots(figsize=(8, 5))
    ax.plot(lrs, losses, linewidth=1.5)
    ax.set_xscale('log')
    ax.set_xlabel('Learning rate (log scale)')
    ax.set_ylabel('Loss (smoothed)')
    ax.set_title(f"LR Range Test — FNO (modes={MODES}, width={WIDTH})")
...
if __name__ == '__main__':
    ...
    print(f"Model: FNO(modes={MODES}, width={WIDTH})")
